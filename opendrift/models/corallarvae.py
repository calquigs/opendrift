...
class CoralLarvae(OceanDrift):
    ...
    required_variables = {
        ...
        'sea_water_temperature': {'fallback': 15, 'profiles': False},
        'sea_water_salinity': {'fallback': 34, 'profiles': False},
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(CoralLarvae, self).__init__(*args, **kwargs)
        ...
        # add config spec
        self._add_config({ 'biology:competency_age_seconds': {'type': 'float', 'default': 0.0,'min': 0.0, 'max': 1.0e10, 'units': 'seconds',
                           'description': 'age in seconds at which larvae become competent for settlement, lose positive buoyancy and begin downward swimmming. Leave as 0 for brooding corals.)',
    ...
    def interact_with_seafloor(self):
            ...
            below_and_older = np.logical_and(self.elements.z < -sea_floor_depth, 
                self.elements.age_seconds >= self.get_config('biology:competency_age_seconds'))
    ...
    def interact_with_coastline(self,final = False): 
        """Coastline interaction according to configuration setting
           ...
           The method checks for age of particles that intersected coastlines:
            ...
            If no specified habitat :
                - if age_particle < competency_age_seconds : move larvaes back to previous wet position
                - if age_particle > competency_age_seconds : larvaes become stranded and will be deactivated.
        ...
        if len(on_land) == 0:
            ...
            if (self.get_config('biology:competency_age_seconds') == 0.0) | (self.get_config('biology:settlement_in_habitat') is True) :
                ...
                # check age of particle versus competency_age_seconds and strand/recirculate accordingly
                on_land_and_younger = np.where((self.environment.land_binary_mask == 1) & (self.elements.age_seconds < self.get_config('biology:competency_age_seconds')))[0]
                on_land_and_older = np.where((self.environment.land_binary_mask == 1) & (self.elements.age_seconds >= self.get_config('biology:competency_age_seconds')))[0]
                ...
                logger.debug('moving %s elements younger than competency_age_seconds back to previous water position' % len(on_land_and_younger))
                logger.debug('%s elements older than competency_age_seconds remain stranded on coast : status = settled_on_coast' % len(on_land_and_older))
                
                # refloat elements younger than competency_age back to previous position(s)
                if len(on_land_and_younger) > 0 :
                    ...
                    self.environment.land_binary_mask[on_land_and_younger] = 0
                # deactivate elements older than competency_age & save position
    ...
    def update(self):
        ...
        self.deactivate_elements(
                self.elements.age_seconds >= self.get_config('drift:max_age_seconds'),
                reason='deactivated')
        self.larvae_buoyancy()
